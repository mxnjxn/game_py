...
# 폰트 정의
game_font = pygame.font.Font(None, 40)  # 폰트 객체를 생성 (폰트 종류, 폰트 크기)
...
# 세로 경계값 처리 2
    ...
    # 타이머 집어 넣기
    # 경과 시간 계산
    # pygame.time.get_ticks() 가 while 문 안에 있기 때문에 계솏해서 시간을 찍어냄
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000
    # pygame.time.get_ticks() --> milisecond 으로 표시 --> 경과 시간을 100-으로 나누어서 초 단위로 표시

    # render 를 한다는게 실제로 글자를 그리는거임
    timer = game_font.render(
        str(int(total_time - elapsed_time)), True, (255, 255, 255))
    # (출력할 글자(str), True, 글자 색상)
